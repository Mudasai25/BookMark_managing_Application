...
const BookmarkManager = () => {
  ...
  const [useAPI, setUseAPI] = useState<boolean>(false); // Toggle for API or local storage
  ...
  useEffect(() => {
    if (useAPI) {
      ...
      const savedBookmarks: Bookmark[] = JSON.parse(localStorage.getItem("bookmarks") || "[]");
  ...
  const submitHandler = (e: React.FormEvent) => {
    ...
    const newBookmark: Bookmark = { title, url };
    if (bookmarks.some((bookmark) => bookmark.url === url)) return; // Avoid duplicates
